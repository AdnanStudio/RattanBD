...
import { calculatePercentange, getChartData, getInventories } from "../utils/features.js";
...
export const getDashboardStats = TryCatch(async(req, res, next)=>{
    ...
    if (myCache.has(key)) {
        ...
        const changePercent = {
            ...
            product: calculatePercentange(thisMonthProducts.length, lastMonthProducts.length),
            user: calculatePercentange(thisMonthUsers.length, lastMonthUsers.length),
            order: calculatePercentange(thisMonthOrders.length, lastMonthOrders.length)
        }
...
export const getPieCharts = TryCatch(async(req, res, next)=>{
    ...
    if (myCache.has(key)) {
        ...
        const discount = allOrders.reduce((prev, order) =>
            prev + (order.discount || 0),
            0
        );

        const productionCost = allOrders.reduce((prev, order) =>
            prev + (order.shippingCharges || 0),
            0
        );
...
export const getBarCharts = TryCatch(async(req, res, next)=>{
    ...
    if (myCache.has(key)) {
        ...
        const productCharts = getChartData({length:6, today, docArr:products});
        const usersCharts = getChartData({length:6, today, docArr:users});
        const ordersCharts = getChartData({length:12, today, docArr:orders});
...
export const getLineCharts = TryCatch(async(req, res, next)=>{
    ...
    if (myCache.has(key)) {
        ...
        const productCharts = getChartData({length:12, today, docArr:products});
        const usersCharts = getChartData({length:12, today, docArr:users});
